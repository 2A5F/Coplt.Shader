...
public static partial class simd_float
{
    ...
    public const i32 EXP_LEN = 8;
    public const i32 SIG_LEN = 23;
    public const i32 EXP_BIAS = (1 << (EXP_LEN - 1)) - 1;
    public const i32 MAX_EXP_BIAS = (1 << EXP_LEN) - 1;
    public const f32 MIN_NORMAL = 1.1754943508222875e-38f; // 0x00800000
    public const f32 MAX_NORMAL = 3.4028234663852886e+38f; // 0x7F7FFFFF
    public const u32 EXP_MASK = 0x7F800000;
    public const u32 MAN_MASK = 0x7FFFFF;
    ...
    [MethodImpl(256 | 512)]
    private static u32 biased_exponent(u32 v) => (v & EXP_MASK) >> SIG_LEN;

    [MethodImpl(256 | 512)]
    private static u32 set_biased_exponent(u32 v, u32 e) => (v & ~EXP_MASK) | ((e << SIG_LEN) & EXP_MASK);

    [MethodImpl(256 | 512)]
    public static f32 Log2Scalar(f32 x)
    {
        // https://github.com/llvm/llvm-project/blob/4ec06b17435e32ece5e1aa2bc8a6d26dbf0bb312/libc/src/math/generic/log2f.cpp

        var m = -EXP_BIAS;
        var x_u = x.asu();
        // log2(1.0f) = 0.0f. 
        if (x_u == 0x3F80_0000) return default;
        if (x is < MIN_NORMAL or > MAX_NORMAL)
        {
            if (x is 0) return f32.NegativeInfinity;
            if (x < 0) return f32.NaN;
            if (x is f32.NaN or f32.PositiveInfinity) return x;
            // Normalize denormal inputs.
            x_u = (x * 8388608f).asu(); // 0x4B000000 ; 0x1.0p23f
            m -= 23;
        }

        m += (i32)biased_exponent(x_u);
        var index = (i32)((x_u & MAN_MASK) >> 16);
        x_u = set_biased_exponent(x_u, 0x7F);

        var u = x_u.asf();
        var v = f32.FusedMultiplyAdd(u, R[index].asf(), -1f); // Exact

        var extra_factor = Log2_R_D[index].asf() + m;

        // Degree-5 polynomial approximation of log2 generated by Sollya with:
        // > P = fpminimax(log2(1 + x)/x, 4, [|1, D...|], [-2^-8, 2^-7]);

        var vsq = v * v; // Exact
        var c0 = f64.FusedMultiplyAdd(v, 0x3ff71547652b8133.asf(), extra_factor);
        var c1 = f64.FusedMultiplyAdd(v, 0x3fdec70a098473de.asf(), 0xbfe71547652d1e33.asf());
        var c2 = f64.FusedMultiplyAdd(v, 0x3fd2514fd90a130a.asf(), 0xbfd7154c5ccdf121.asf());

        var r = f64.FusedMultiplyAdd(vsq, f64.FusedMultiplyAdd(vsq, c2, c1), c0);
        return (f32)r;
    }
